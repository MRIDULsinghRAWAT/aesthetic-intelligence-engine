...
This document provides a comprehensive presentation guide to showcase the **Aesthetic Intelligence Engine** to your mentor, followed by a complete mapping of all major features to their exact file and line-level implementation details in the codebase.

---

## Part 1: How to Give a Stunning Demo to Your Mentor

### 1. The 30-Second Elevator Pitch
> *"The Aesthetic Intelligence Engine is an on-device Edge AI solution that evaluates and enhances image aesthetic quality in real time. We compressed a MobileNetV2 model by 75% down to 2.54 MB using Dynamic Range Quantization, achieving sub-5ms CPU scoring on edge hardware with zero cloud latency and full data privacy. It also includes an automated OpenCV enhancement pipeline and a live MLOps hot-swapping controller with MLflow telemetry tracking."*

---

### 2. Pre-Demo Setup Commands (Run These First)

Open two terminal windows in the project directory (`c:\Users\Mridul\Desktop\aesthetic-intelligence-engine`):

#### Terminal 1: Launch Web Application
```bash
python app.py
```
* **URL**: `http://localhost:5000` (auto-opens in your browser)
* **Mobile / Remote Access**: Binds to `0.0.0.0:5000` so anyone on the same Wi-Fi can open `http://<YOUR_LOCAL_IP>:5000` on their phone camera.

#### Terminal 2: Launch MLflow Telemetry Dashboard
```bash
mlflow ui --backend-store-uri sqlite:///mlflow.db --port 5001
```
* **URL**: `http://localhost:5001` (shows live metric logging, prediction latency, and active model versions).
...
4. **Point out to your mentor**:
   - **Score**: Numerical aesthetic value from `0.00` to `1.00`.
   - **Classification Badge**: *"Good Aesthetic"* vs *"Bad Aesthetic"*.
   - **Sub-5ms Latency**: Real-time evaluation running locally on CPU.
...
1. For an image with low contrast or low aesthetic score, click **"Auto-Enhance Image"**.
...
#### Step 4: Live MLOps Hot-Swapping & Dynamic Model Rollbacks
1. Scroll down to the **MLOps Model Controller** card on the dashboard.
2. Click **"Deploy v2.0.0 (Upgrade)"**.
   - Notice the toast confirmation and how the active model version updates to `v2.0.0` live without restarting the Flask server!
3. Click **"Rollback to v1.0.0"**.
   - Watch the dynamic interpreter re-allocate back to `v1.0.0` in real time.
4. **Point out to your mentor**: Complete thread-safe dynamic interpreter swapping while the server remains 100% active.

#### Step 5: Live MLflow Telemetry Audit
1. Switch to `http://localhost:5001` (MLflow UI).
2. Show the **"aesthetic-intelligence-engine"** experiment runs.
3. **Point out to your mentor**: Every prediction logs prediction score, latency in milliseconds, enhancement status, and exact TFLite model version used into `mlflow.db`.

---

## Part 2: Important Features & Code Implementation Mapping

Below is the complete architectural mapping showing where every key technical feature is implemented in the codebase.
